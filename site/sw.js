/* ==========================================================================
   sw.js â€” the service worker. Makes the whole tutorial work offline.
   ...
   API call in the reading path â€” it is *already* offline-capable in every way
   ...
   the network and should fail honestly when it is not there â€” a cached 200 for
   ...
   of this silently, which is correct â€” nothing to cache, nothing to serve â€” and
...
/* The chapter list, imported rather than duplicated. chapters.js assigns onto
   `self`, which is why that works here â€” see the note at the bottom of it. */
...
/* BUMP THIS ON EVERY CONTENT CHANGE.
   ...
   mechanism â€” there is no partial invalidation and you do not want one. Forget
...
const CACHE = "logiflow-academy-v29";
...
/* Fetch one URL and store it under the URL WE ASKED FOR, as a fresh response.
   ...
   then REFUSES to use that response for a navigation â€” "a redirected response
   was used for a request whose redirect mode is not follow" â€” so the first
...
async function precache(cache, url) {
  try {
    ...
    /* Reported, not swallowed. The caller decides what a failure means: fatal
       for the shell, survivable for one chapter â€” see install() below. */
  ...
  event.waitUntil(
    (async () => {
      ...
      /* THE SHELL IS REQUIRED, AND A FAILURE HERE MUST FAIL THE INSTALL.
         ...
         always reported success â€” one flaky response out of ninety simultaneous
      ...
      /* Take over as soon as the install finishes rather than waiting for every
         tab to close â€” and only now, with the shell verified present. Safe here
...
self.addEventListener("fetch", (event) => {
  ...
  event.respondWith(
    caches.match(request).then((cached) => {
      /* CACHE FIRST, with a background refresh â€” "stale while revalidate".
